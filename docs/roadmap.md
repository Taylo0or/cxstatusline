...
- Model, reasoning, service tier, permission, sandbox, session, event, run-state,
  ...
  upstream-compatible status format, raw/default labels, Skills metadata, Nerd
  Font, and compaction hide-zero controls where applicable.
- Token speed, input speed, output speed, total speed, usage, weekly usage,
  ...
  cursor display metadata, Context Bar detail display metadata, reset timer
  progress/slider metadata, and extra usage disabled-state hiding.
...
- Full-screen TUI configuration editor with live preview, widget picker, line
  ...
  PR/MR status/title, Git empty-state, usage display, extra usage disabled
  hiding, status-format, Nerd Font, Jujutsu empty-state, and remote empty-state
  toggles,
